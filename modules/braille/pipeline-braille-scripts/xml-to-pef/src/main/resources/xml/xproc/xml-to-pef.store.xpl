...
<p:declare-step type="px:xml-to-pef.store" version="1.0"
    ...
    <p:count/>
    <px:message>
        <p:with-option name="message" select="concat('[progress px:dtbook-to-pef.store ',(if (number(string(/*)) &gt; 0) then '90' else '100'),' pef:store] Storing PEF',(if ($include-brf='true') then ', BRF' else ''),(if ($include-preview='true') then ' and HTML preview' else ''))"/>
    </px:message>
    <p:choose>
