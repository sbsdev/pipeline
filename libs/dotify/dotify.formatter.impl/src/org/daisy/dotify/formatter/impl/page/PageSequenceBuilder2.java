...
public class PageSequenceBuilder2 {
	...
	public BlockContext getContext() {
		...
		// System.err.println("PageSequenceBuilder2.getContext()");
		...
		// System.err.println(" --> " + bc.getRefs());
		
		return bc;
	}
	
	private void modifyContext(Consumer<BlockContext.Builder> modifier) {
		
		// System.err.println("PageSequenceBuilder2.modifyContext()");
	...
	// -> by adding a "context" argument to nextPage() ?
	public void modifyRefs(Consumer<CrossReferenceHandler.Builder> modifier) {
		
		// System.err.println("PageSequenceBuilder2.modifyRefs()");
	...
	public boolean hasNext() {
		...
		// System.err.println("PageSequenceBuilder2.hasNext()");
	...
	public PageImpl nextPage(int pageNumberOffset, boolean hyphenateLastLine, Optional<TransitionContent> transitionContent) throws PaginatorException, RestartPaginationException // pagination must be restarted in PageStructBuilder.paginateInner
	{
		...
		// System.err.println("PageSequenceBuilder2.nextPage()");
		...
		while (dataGroupsIndex<dataGroups.size() || (data!=null && !data.isEmpty())) {
			...
			if (!data.isEmpty()) {
				...
				SplitPointDataSource<RowGroup> seqTransitionText = transitionContent.isPresent()
						// FIXME:
						// -> which state of refs to use for this? first split page and then pass refs?
						// -> two passes needed because refs also needed to compute height
						// -> is there even a difference?
						? new RowGroupDataSource(master, getContext(), transitionContent.get().getInSequence(), null, cd)
				...
				// FIXME: isn't this reset done automatically in RowGroupDataSource?
				...
				if (addTransition && transitionContent.isPresent()) {
					if (transitionContent.get().getType()==TransitionContent.Type.INTERRUPT) {
						...
						for (SplitPointDataSource.Iterator<RowGroup> it = seqTransitionText.iterator(); it.hasNext();) {
